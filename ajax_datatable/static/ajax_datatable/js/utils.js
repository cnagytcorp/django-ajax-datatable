...
window.AjaxDatatableViewUtils = (function () {
  ...
  function _handle_column_filter(table, data, target) {
    ...
    console.log(
      "Request to search value %o in column %o (current value: %o)",
      value,
      index,
      old_value
    );
  ...
  function _setup_column_filters(table, data) {
    if (data.show_column_filters) {
          ...
          if (item.searchable) {
            ...
            if ("choices" in item && item.choices) {
              ...
              var input = $("<input>")
                ...
                .attr(
                  "value",
                  item.initialSearchValue ? item.initialSearchValue : ""
                );
              html = $("<div>").append(input).html();
            }
            if (item.className) {
              filter_row +=
                '<th class="' + item.className + '">' + html + "</th>";
            } else {
            ...
            if (index == 0) {
              ...
              var search_icon_html =
                _options.search_icon_html === undefined
                  ? ""
                  : _options.search_icon_html;
      ...
      var wrapper = table.closest(".dataTables_wrapper");
      $(filter_row).appendTo(wrapper.find("thead"));
  ...
  function _bind_row_tools(table, url, options, extra_data) {
    ...
    if (options.full_row_select) {
      ...
      table.api().on("click", "td", function (event) {
        ...
        if (row.child.isShown()) {
          ...
          if (!tr.hasClass("details")) {
            row
              .child(_load_row_details(row.data(), url, extra_data), "details")
              .show("slow");
            tr.addClass("shown");
          }
        }
      ...
      table
        ...
        .on(
          "click",
          "td.dataTables_row-tools .plus, td.dataTables_row-tools .minus",
          function (event) {
            event.preventDefault();
            var tr = $(this).closest("tr");
            var row = table.api().row(tr);
    ...
    $.ajax({
      ...
      var options = {
        ...
        ajax: function (data, callback, settings) {
          ...
          $.ajax({
            ...
            .fail(function (jqXHR, textStatus, errorThrown) {
              alert("Unable to handle request, reloading page!");
              console.log("ERROR: " + jqXHR.responseText);
              location.reload(); // instead of fail silently, display message and reload page.
            });
        },
        ...
        initComplete: function () {
          ...
          table.trigger("initComplete", [table]);
        },
        drawCallback: function (settings) {
          // Notify subscribers
          //console.log('Broadcast drawCallback()');
          table.trigger("drawCallback", [table, settings]);
        },
        rowCallback: function (row, data) {
          // Notify subscribers
          //console.log('Broadcast rowCallback()');
          table.trigger("rowCallback", [table, row, data]);
        },
        footerCallback: function (row, data, start, end, display) {
          // Notify subscribers
          //console.log('Broadcast footerCallback()');
          table.trigger("footerCallback", [
            table,
            row,
            data,
            start,
